...
bool run(edumip_balance_ros::keycomm::Request  &req, edumip_balance_ros::keycomm::Response &res)
{
	int a = req.key;
	commander = a;
    //ROS_INFO("commander update: %d",commander);
	/*
	if (a == 1)
	{
...
int main(int argc, char** argv)  
{
  ...
  ros::init(argc, argv, "edumip_balance_ros_node_key");
  ...
  // Create nodehandle
  ros::NodeHandle edumip_node1;

  // Advertise the topics this node will publish
  state_publisher = edumip_node1.advertise<edumip_msgs::EduMipState>("edumip/state", 10);

  // subscribe the function CmdCallback to the topuc edumip/cmd
  ros::Subscriber sub_cmd = edumip_node1.subscribe("edumip/cmd", 10, CmdCallback);

//TGH1 added: //This initialization is for the bot to subscrible to topic "keyhitinfo" which has the info to actuate the bot.
	ros::ServiceServer service = edumip_node1.advertiseService("keyhit_info", run);
 ...
 */
 ...
 //keyboard actuator   
	if (commander != 0)
	{
		...
		{
		case 1:
		setpoint.phi_dot = DRIVE_RATE_NOVICE;
		break;
		case 3:
		setpoint.phi_dot =- DRIVE_RATE_NOVICE;
		break;
