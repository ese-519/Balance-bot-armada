...
#include "edumip_balance_ros/instruction.h"
...
double phi_got = 0;
double gamma_got =0;
// Function to run for ros spin() and bot actuation 
bool run(edumip_balance_ros::instruction::Request  &req, edumip_balance_ros::instruction::Response &res)
{
	phi_got = req.phi;
	gamma_got = req.gamma;
	res.resp = 1;
	//ROS_INFO("response status %d",(int)res.resp);
	return true;
}
...
int main(int argc, char** argv)  
{
  ...
  ros::init(argc, argv, "edumip_balance_ros_node_bot2");
  ...
  // Create nodehandle
  ros::NodeHandle edumip_node2;

  // Advertise the topics this node will publish
  state_publisher = edumip_node2.advertise<edumip_msgs::EduMipState>("edumip/state", 10);

  // subscribe the function CmdCallback to the topuc edumip/cmd
  ros::Subscriber sub_cmd = edumip_node2.subscribe("edumip/cmd", 10, CmdCallback);

//TGH1 added: //This initialization is for the bot to subscrible to topic "bot1follower" which has the info to actuate the bot.
	ros::ServiceServer service = edumip_node2.advertiseService("bot2follower", run);
 ...
 */
 ...
 //keyboard actuator   
		setpoint.phi_dot = DRIVE_RATE_NOVICE * phi_got;
		setpoint.gamma_dot = TURN_RATE_NOVICE * gamma_got;
	}		
